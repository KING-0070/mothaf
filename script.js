...
function applyRevealStagger() {
    const gridSelectors =
        '.unesco-grid, .home-cards-grid, .visiting-grid, .halls-grid, .exhibits-detailed, .gallery-grid, .gallery-grid--compact, .tickets-layout';

    document.querySelectorAll(gridSelectors).forEach((grid) => {
        const items = grid.querySelectorAll(
            '.reveal, .reveal-fade, .reveal-scale, .reveal-left, .reveal-right, .visit-card, .gallery-item, .exhibit-detailed-card, .unesco-card, .home-card, .hall-card, .tickets-info, .tickets-form'
        );
        items.forEach((el, i) => {
